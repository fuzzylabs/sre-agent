...
class ServiceMonitor:
    ...
    async def start_monitoring(self, watch: bool = False, interval: int = 30, 
                              services: Optional[List[str]] = None, max_duration: Optional[int] = None):
    ...
    async def _watch_mode(self, interval: int, services: Optional[List[str]], 
                         max_duration: Optional[int], start_time: float):
            ...
            with Live(create_status_display(), console=console, refresh_per_second=1) as live:
                ...
                while self.monitoring:
                    ...
                    if max_duration and (time.time() - start_time) > max_duration:
                        console.print(f"\n[yellow]Maximum monitoring duration ({max_duration}s) reached.[/yellow]")
                        break
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {"system": {"healthy": False, "issues": [f"API returned {response.status_code}"], "last_check": datetime.now().strftime("%H:%M:%S")}}
                    
        except httpx.ConnectError:
            return {"system": {"healthy": False, "issues": ["Cannot connect to SRE Agent API"], "last_check": datetime.now().strftime("%H:%M:%S")}}
        except Exception as e:
            return {"system": {"healthy": False, "issues": [str(e)], "last_check": datetime.now().strftime("%H:%M:%S")}}
    
    def _parse_health_response(self, health_data: dict, services: Optional[List[str]]) -> Dict[str, Any]:
        ...
        if services:
            ...
            for service in services:
                result[service] = {
                    ...
                    "issues": [] if health_data.get("status") == "healthy" else ["Service status unknown"],
            ...
            return {
                "system": {
                    "healthy": health_data.get("status") == "healthy",
                    "issues": [] if health_data.get("status") == "healthy" else health_data.get("issues", ["Unknown issues"]),
                    "last_check": current_time
                }
            }
    ...
    def _display_status(self, status: Dict[str, Any]):
        ...
        for service_name, service_status in status.items():
            ...
            if is_healthy:
                panel_style = "green"
                status_text = "✅ Healthy"
                content = f"[green]{status_text}[/green]\n\nLast check: {service_status.get('last_check', 'Unknown')}"
            else:
                panel_style = "red"
                status_text = "❌ Issues Detected"
                issues_text = "\n".join([f"• {issue}" for issue in issues])
                content = f"[red]{status_text}[/red]\n\n[yellow]Issues:[/yellow]\n{issues_text}\n\nLast check: {service_status.get('last_check', 'Unknown')}"
...
@click.pass_context
def monitor(ctx, watch: bool, namespace: Optional[str], cluster: Optional[str], 
           interval: Optional[int], services: tuple, duration: Optional[int], 
           output: Optional[str]):
    """
    ...
    if watch:
        console.print(f"[cyan]Starting continuous monitoring (Ctrl+C to stop)...[/cyan]")
        if duration:
